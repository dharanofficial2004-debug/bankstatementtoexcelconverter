...
export default function sitemap(): MetadataRoute.Sitemap {
  const baseUrl = "https://bankstatementtoexcelconverter.com/ar-jo";
  const lastModified = new Date();

  const srcAppDir = path.join(process.cwd(), "src", "app", "ar-jo");
  const rootAppDir = path.join(process.cwd(), "app", "ar-jo");
  const appDir = fs.existsSync(srcAppDir) ? srcAppDir : rootAppDir;

  const dynamicRoutes = getRoutes(appDir);

  return dynamicRoutes.map((route) => ({
    url: route === "" ? baseUrl : `${baseUrl}/${route.replace(/\\/g, "/")}`,
    lastModified,
